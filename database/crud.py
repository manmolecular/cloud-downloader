...
class DbManager:
    ...
    def create_user(self, user: schemas.User, session: Session = SessionLocal()):
        ...
        user_meta_as_dict.update({"password": sha512(user_meta_as_dict.get("password").encode()).hexdigest()})
    ...
    def check_credentials(self, user: schemas.User, session: Session = SessionLocal()):
        ...
        user_meta_as_dict.update({"password": sha512(user_meta_as_dict.get("password").encode()).hexdigest()})
        user_model = models.User(**user_meta_as_dict)
        user_db = session.query(models.User).filter(models.User.username == user_model.username).first()
    ...
    def get_task_status(self, uuid: str, session: Session = SessionLocal()):
        return session.query(models.Task).filter(models.Task.uuid == uuid).first().status

    def update_task_status(self, uuid: str, status: str, session: Session = SessionLocal()):
    ...
    def get_user_id(self, username: AnyStr, session: Session = SessionLocal()):
        return session.query(models.User).filter(models.User.username == username).first().password
